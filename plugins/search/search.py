...
class TreeSitterSearch(CodeSearchPlugin):
        ...
        try:
            ...
            if filter_paths:
                ...
                for pattern in filter_paths:
                    ...
                    filtered_files.update(f for f in files if pattern in f.lower())
    ...
    async def _parse_query(self, query: str) -> Dict[str, Any]:
        ...
        for type_name, pattern in type_patterns.items():
            if re.search(
                pattern, query
            ):  # No need for re.I flag as it's in the pattern
        ...
        for modifier, pattern in modifier_patterns.items():
            if re.search(
                ...
                if isinstance(components["modifiers"], list):
                    components["modifiers"].append(modifier)

        # Extract context keywords (already case insensitive due to lowered query)
        query_words = set(re.findall(r"\b\w+\b", query))
        context_words = query_words - set(
            sum([p.split("|") for p in type_patterns.values()], [])
        )
        context_words = context_words - set(
            sum([p.split("|") for p in modifier_patterns.values()], [])
        )
    ...
    async def parse_query(self, query: str) -> Dict[str, Any]:
        """Public interface for query parsing."""
        return await self._parse_query(query)
        ...
        try:
            ...
            if "todo" in query:
                return await self.semantic_search.search(
                    query, filter_paths=filter_paths, min_score=min_score, limit=limit
                )
            ...
            parsed_query = await self.syntax_search.parse_query(query)
            if parsed_query["type"] in ["function", "class"]:
                ...
                unique_results: List[SearchDict] = []
